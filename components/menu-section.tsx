    ...
    <section id="menu" className="mx-auto max-w-6xl px-4 py-16 md:py-24">
      ...
      <div className="grid gap-8 md:grid-cols-2">
        {t.menu.categories.map((category) => {
          const isLong = category.items.length > COLLAPSED_COUNT
          const isExpanded = expanded[category.name] ?? false
          const visibleItems =
            isLong && !isExpanded
              ? category.items.slice(0, COLLAPSED_COUNT)
              : category.items
            ...
            >
              ...
              {isLong && (
                <button
                  ...
                  className="flex items-center justify-center gap-1.5 border-t border-border px-5 py-3 text-sm font-bold text-primary transition-colors hover:bg-muted"
                >
